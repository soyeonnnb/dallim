...
public class JwtFilter extends OncePerRequestFilter { // 모든 요청에 대해 토큰 유효성 검증을 진행
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
        ...
        return path.startsWith("/swagger-ui/")||path.startsWith("/swagger-resources")||path.startsWith("/v2/api-docs")||path.startsWith("/api/oauth/login") || path.startsWith("/login/") || path.startsWith("/api/oauth2/authorization/") ||
            path.startsWith("/api/login/oauth2/") || path.startsWith("/api/oauth/logout") || path.startsWith("/favicon.ico")||path.startsWith("/api/oauth2/code/kakao")||path.startsWith("/api/oauth2/code/naver")||path.startsWith("/api/v1/user/token");
    }
